...
def run_technical(
    ...
    *,
    ...
    _ask_requirements(ask, doc, expected_users)
    recommend_stack(doc)
    ...
    if mode is not SelectionMode.RECOMMEND_ALL:
        _offer_overrides(ask, doc)
    return doc


def _ask_requirements(
    ask: ui.Prompter, doc: TechnicalSpec, expected_users: int | None
) -> None:
    req = doc.requirements
    ui.say()
    ui.info("[bold]필요한 것을 확인합니다[/bold]")

    req.needs_external_materials = ask.ask_yes_no(
        "AI가 별도의 교과서나 학습자료를 참고해야 합니까?",
        default=req.needs_external_materials,
        hint="'참고해야 한다'면 자료를 찾아오는 기능이 필요합니다. 일반 지식으로 충분하면 아니오.",
        allow_unknown=True,
    )
    req.needs_persistent_learner_memory = ask.ask_yes_no(
...
def _offer_overrides(ask: ui.Prompter, doc: TechnicalSpec) -> None:
    ui.say()
    change = ask.ask_yes_no("바꾸고 싶은 것이 있나요?", default=False)
    if not change:
        return
    ui.note(
