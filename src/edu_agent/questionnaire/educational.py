...
def run_educational(doc: EducationalDesign, *, tasks_dir: Path | None = None) -> EducationalDesign:
    ...
    _ask_ai_usage(doc)
    _ask_support(doc)
    _ask_tasks(doc, tasks_dir)
    return doc


def _ask_context(doc: EducationalDesign) -> None:
    ui.say()
    ui.info("[bold]1/7 · 학습 맥락[/bold]")
    ctx = doc.context
    ctx.target_learners = ui.ask_text(
        "누가 이 AI를 사용하나요?",
        default=ctx.target_learners,
        hint="대상이 정해져야 AI의 말투·설명 수준·안전 기준이 정해집니다.",
    )
    ctx.age_or_grade = ui.ask_text("연령이나 학년은 어떻게 되나요?", default=ctx.age_or_grade)
    ctx.subject = ui.ask_text("어떤 교과나 영역인가요?", default=ctx.subject)
    ctx.topic = ui.ask_text("구체적인 학습 주제는 무엇인가요?", default=ctx.topic)
    ctx.usage_situation = ui.ask_text(
        "언제 사용하나요? (수업 중, 과제할 때, 스스로 공부할 때 등)",
        default=ctx.usage_situation,
    )
    ctx.expected_users = ui.ask_int(
        "몇 명 정도가 사용할 예정인가요?",
...
def _ask_objectives(doc: EducationalDesign) -> None:
    ui.say()
...
def _ask_ai_usage(doc: EducationalDesign) -> None:
    ui.say()
    ...
    usage.before_ai = ui.ask_text("AI를 쓰기 전에 학습자는 무엇을 하나요?", default=usage.before_ai)
    usage.after_ai = ui.ask_text("AI를 쓴 뒤에 학습자는 무엇을 하나요?", default=usage.after_ai)


def _ask_support(doc: EducationalDesign) -> None:
    ui.say()
    ...
    while len(support.must_do) < 8:
        item = ui.ask_text(f"필수 행동 {len(support.must_do) + 1} (없으면 엔터)")
        if not item:
            break
    ...
    while len(support.must_not_do) < 8:
        item = ui.ask_text(f"금지 행동 {len(support.must_not_do) + 1} (없으면 엔터)")
        if not item:
            break
        support.must_not_do.append(item)


def _ask_tasks(doc: EducationalDesign, tasks_dir: Path | None) -> None:
    ui.say()
    ...
    add = ui.ask_yes_no("지금 과제를 등록하시겠어요?", default=len(doc.tasks) == 0)
    if not add:
        return
    ...
    while len(doc.tasks) < 10:
        n = len(doc.tasks) + 1
        title = ui.ask_text(f"과제 {n}의 제목 (없으면 엔터)")
        if not title:
            break
        tid = f"t{n:02d}"
        while tid in used:
        ...
        misconception = ui.ask_text("  이 과제에서 학생들이 흔히 갖는 오개념이 있나요? (선택)")
        doc.tasks.append(
            TaskItem(
                id=tid,
                title=title,
