...
def test(
    ...
    judge = Judge(judge_provider, order_swap=project.config.evaluation.order_swap) if judge_provider else None
    report = evaluate(
        EvaluationInput(
...
def _regrade(project: Project, spec, run_id: str, *, no_llm: bool, mock: bool) -> None:
    ...
    judge_provider = _provider(project, role="judge", no_llm=no_llm, mock=mock)
    judge = Judge(judge_provider) if judge_provider else None
    report = evaluate(
...
def _attach_calibration(project: Project, report) -> None:
    ...
    report.calibration = compute_calibration(
        ratings, judge_labels, threshold=project.config.evaluation.calibration_threshold
    )
...
def calibrate(
    ...
    if submit:
        ...
        if not report_path.exists():
            ui.info("평가 결과 파일이 없습니다. edu-agent test 를 먼저 실행하세요.")
            return
