...
**누가 쓰나** — 자기 수업에 쓸 에이전트를 만드는 교사, 한 학기 과제로 만드는
수강생, 설계원리가 실제 행동으로 옮겨졌는지 측정하려는 연구자. 세 경우 모두
같은 도구를 같은 방식으로 씁니다. **혼자 쓰신다면 아래 5분 안내만 따라오시면 됩니다.**
...
여러 사람에게 이 도구로 과제를 내실 때만 필요한 자료입니다. 혼자 에이전트를
만드신다면 **이 절은 건너뛰셔도 됩니다.** 하네스는 수업을 알지 못하고,
여기 있는 것을 하나도 안 써도 똑같이 동작합니다.

[`course/`](course/) 에 있는 것:

- [roadmap.md](course/roadmap.md) — 15주 로드맵 (12주·8주로 줄이는 방법 포함)
- [api-keys.md](course/api-keys.md) — API 키 배포 전략
- [rubric.md](course/rubric.md) — 교수자용 평가 루브릭
- [troubleshooting.md](course/troubleshooting.md) — 처음 쓰는 사람이 자주 막히는 지점
...
- ✅ 시뮬레이터: 페르소나 8종, 인식 상태 분리, 다중 시드, 과목을 아는 학생(정답은 모르는)
