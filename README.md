...
- ✅ 4개 문서의 스키마·템플릿·대화형 작성 (`init`, `review`, `status`)
- ✅ 문헌 근거가 붙은 설계원리 라이브러리 10개
- ✅ 자연어 원리 → 실행 구조 변환 (사용자 확인 필수)
- ✅ 컴파일러: 결정적 검증 + 게이트 생성 + 추적성 + 시나리오 생성
- ✅ 런타임: 학습자 상태 추적, 정책 게이트, 정답 누출 검사, JSONL trace
- ✅ 시뮬레이터: 페르소나 8종, 인식 상태 분리, 다중 시드
- ✅ 평가: 자동 검사 12종 + AI 판정 루브릭 12종 + 학생 측 지표 + 보정
- ✅ 개선 루프: 진단 → 제안 → 승인 → 재검증 → 회귀 게이트 → 롤백
- ✅ 보고서, 내보내기(CLI), 예제 프로젝트
- ✅ 테스트 218개, Windows/macOS/Linux CI

## 다음 단계

- `--web` 로컬 웹 채팅 (gradio)
- 코드 실행 도구의 실제 샌드박스 구현
- FastAPI 내보내기
- 사람 채점 결과로 judge 프롬프트를 개선하는 순환
- 영어 문서 템플릿
